  ...
  return (
    <div>
      <h2 id="employee-heading" data-cy="EmployeeHeading">
        <Translate contentKey="jhipsterSampleApplication12345App.employee.home.title">Employees</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterSampleApplication12345App.employee.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/employee/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterSampleApplication12345App.employee.home.createLabel">Create new Employee</Translate>
          </Link>
        </div>
      </h2>
        ...
        {employeeList && employeeList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipsterSampleApplication12345App.employee.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <th className="hand" onClick={sort('pict')}>
                  <Translate contentKey="jhipsterSampleApplication12345App.employee.pict">Pict</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <th className="hand" onClick={sort('cv')}>
                  <Translate contentKey="jhipsterSampleApplication12345App.employee.cv">Cv</Translate> <FontAwesomeIcon icon="sort" />
                </th>
            ...
            <tbody>
              {employeeList.map((employee, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {employee.hireDateTime ? <TextFormat type="date" value={employee.hireDateTime} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                    {employee.zonedHireDateTime ? (
                      <TextFormat type="date" value={employee.zonedHireDateTime} format={APP_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>{employee.hireDate ? <TextFormat type="date" value={employee.hireDate} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{employee.salary}</td>
                  <td>{employee.commissionPct}</td>
                  <td>{employee.duration ? <DurationFormat value={employee.duration} /> : null}</td>
                  <td>
                    {employee.pict ? (
                      <div>
                  ...
                  <td>{employee.manager ? <Link to={`/employee/${employee.manager.id}`}>{employee.manager.fullName}</Link> : ''}</td>
                  <td>
                    {employee.department ? (
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterSampleApplication12345App.employee.home.notFound">No Employees found</Translate>
            </div>
          )
        )}
      </div>
